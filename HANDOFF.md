...
Current release line v0.2.2; updated 2026-07-21 for the Windows recovery and complete localized-chrome hotfix. Audience: the next implementer (a coding agent or human) taking over development. When this file disagrees with the code, the code wins — then fix this file.
...
- The manifests report **0.2.2**. Accurate earlier history: v0.1.0 predates Windows packaging and has Linux + macOS artifacts with no notes; v0.1.1–v0.1.3 have all four platforms but no notes; v0.1.4 onward have four-platform artifacts and bilingual notes.
...
- CI runs on ubuntu + windows: version sync → build → tests → typecheck → Linux evidence suite → real-browser smoke. The v0.2.2 release snapshot passes production build, **50 files / 348 tests**, full typecheck, all four evidence instruments, and real Chromium locally on Linux; smoke now also creates a zh-TW workspace and rejects leaked English built-in names.
...
- `server/src/providers/install.ts` — fixed install recipes per provider/platform plus fixed time limits (Agy/winget 10 minutes; npm recipes 5). `POST /api/providers/refresh` accepts no user data, clears augmented-PATH/version caches, and reruns discovery. `providers/auth.ts` — auth-alert store + verified sign-in command table.
