...
Published release v0.2.1; updated 2026-07-21 for the provider-recovery and localized-UI release. Audience: the next implementer (a coding agent or human) taking over development. When this file disagrees with the code, the code wins — then fix this file.
...
- **v0.2.1 is published**. Accurate earlier history: v0.1.0 predates Windows packaging and has Linux + macOS artifacts with no notes; v0.1.1–v0.1.3 have all four platforms but no notes; v0.1.4 onward have four-platform artifacts and bilingual notes.
- The manifests report **0.2.1**. This release was driven by `mat-debug-r_DDtz7R3rmDUzSymdyYvIA`: Windows Codex/Grok cold probes timed out after 5 seconds and the failures were then cached for 10 minutes; Agy needed up to five minutes and the UI hid the successful completion. The fix gives Windows version probes 15 seconds (8 elsewhere), caches failures only 2 seconds, adds an explicit cache-bypassing provider recheck, allows Agy installs 10 minutes, and keeps elapsed/probe/restart guidance visible after install.
- v0.2.1 also adds persisted system/en/zh-TW UI language and three persisted themes (Midnight, Daylight, violet/gold/teal Aurora), visible text next to navigation icons, and stronger actor/provider identity in Conversation cards. This was based on the latest `multi-ai-chat-desktop` head (`4e98c06`) without importing its runtime or assets.
- CI runs on ubuntu + windows: version sync → build → tests → typecheck → Linux evidence suite → real-browser smoke. The release snapshot passes production build, **48 files / 328 tests**, full typecheck, all four evidence instruments, and real Chromium locally on Linux; the smoke explicitly exercises visible nav labels, zh-TW, all three theme choices, and preference persistence.
...
- `server/src/adapters/base.ts` — spawn/probe plumbing; version probes allow 15 s on Windows / 8 s elsewhere, share in-flight work, cache successes 10 minutes and transient failures only 2 seconds; `providerSpawnSlot()` per-provider promise chain spacing same-provider spawns ≥1500 ms (mock exempt); `detectAuthFailure()` pattern-scans the last 4 KB of output and composes `errorReason` + `Fix: <signInCommand>` + the CLI's own instruction line (api-key **values** filtered out).
...
- `web/src/i18n/UiPreferences.tsx` + `app/theme.css` — typed English/Traditional-Chinese copy, system-locale resolution, and three semantic-token themes. Preferences persist in `mat-ui-preferences-v1`; `main.tsx` applies them before React mounts to avoid a theme flash. Standalone component tests deliberately fall back to English. Add new visible copy to both dictionaries; do not regress Light/Aurora by reintroducing neutral `zinc` surface/text classes.
