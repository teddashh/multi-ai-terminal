...
export function healthFindings(input: HealthInput): HealthFinding[] {
  return [
    ...
    ...(input.providers.length > 0 ? input.providers.map((provider) => providerFinding(provider, input.locale)) : [{
      ...
      title: copy(input.locale, 'No provider status loaded', '尚未載入 provider 狀態'),
...
export function providerFinding(provider: ProviderInfo, locale: HealthLocale = 'en'): HealthFinding {
  if (provider.id === 'mock') {
    return {
      ...
      title: copy(locale, 'mock: deterministic test provider', 'mock：固定結果的測試 provider'),
      detail: copy(locale, 'Built-in and intentionally exempt from CLI discovery and authentication checks.', '此 provider 內建於 MAT，刻意不進行 CLI 偵測與登入檢查。'),
    };
  }
  if (!provider.ok) {
    ...
    return {
      ...
      detail: `${provider.detail ?? copy(locale, 'MAT could not find this provider executable on its augmented PATH.', 'MAT 在擴充後的 PATH 中找不到此 provider 執行檔。')}${recordedAuthFailure}\n${copy(locale, 'Transient failures are only cached briefly; use retry detection to bypass all provider caches.', '暫時性失敗只會短暫快取；可用「重新偵測」略過所有 provider 快取。')}`,
    };
  }
...
function serverFindings(server: ServerHealthState, wsConnection: WsConnectionState, providerRefreshError?: string, locale: HealthLocale = 'en'): HealthFinding[] {
  ...
  if (server.status === 'checking') {
    ...
    findings.push({ id: 'server', scope: 'system', severity: 'ok', issue: false, title: copy(locale, 'Local server reachable', '本機伺服器可連線'), detail: server.version ? `MAT server ${server.version}` : copy(locale, 'The health request succeeded.', '健康狀態請求成功。') });
  }
  ...
  if (providerRefreshError) {
    findings.push({ id: 'provider-refresh', scope: 'system', severity: 'warning', issue: true, title: copy(locale, 'Provider check could not refresh', '無法重新整理 provider 檢查'), detail: providerRefreshError });
  }
  return findings;
}
  ...
  return [
    {
      ...
      detail: `${source}: ${name}. ${isGit
        ? copy(locale, 'This makes worktree isolation eligible; MAT checks the repository again when a stage runs, and patch evidence exists only when a candidate creates it.', '可使用 worktree 隔離；MAT 會在階段執行時再次檢查 repository，且只有 candidate 真的建立 patch 時才會有 patch 證據。')
        : copy(locale, 'This is supported; worktree stages fall back to the workspace and do not produce worktree patch evidence.', '此狀態受支援；worktree 階段會回退到原工作區，且不會產生 worktree patch 證據。')}`,
    },
    {
      ...
      detail: verifyCommand
        ? `${source}${copy(locale, ' has a verification command. MAT runs it only for a successful worktree-isolated candidate with a non-empty patch.', '已設定驗證指令。MAT 只會對成功、採 worktree 隔離且 patch 非空的 candidate 執行。')}`
...
function runFindings(run: RunSnapshot | undefined, integrity: EvidenceIntegrity | undefined, locale: HealthLocale = 'en'): HealthFinding[] {
  ...
  const findings: HealthFinding[] = [{
    ...
    title: copy(locale, `Viewed run: ${run.status}`, `目前執行：${translatedRunStatus(run.status, locale)}`),
  ...
  if (integrity?.status === 'recovering') {
    ...
    findings.push({
      ...
      detail: sequenceDetail(integrity, integrity.message ?? copy(locale, 'One or more persisted events could not be recovered.', '有一個或多個持久化事件無法復原。'), locale),
    });
  ...
  for (const node of run.nodes) {
    ...
    if (node.status === 'failed' || node.status === 'stalled') {
      findings.push({
        ...
        title: `${label}: ${locale === 'zh-TW' ? translatedRunStatus(node.status, locale) : node.status}`,
  ...
  for (const decision of run.gateDecisions.filter((candidate) => candidate.degraded)) {
    ...
    findings.push({
      ...
      title: copy(locale, `${stageName}: degraded gate decision`, `${stageName}：gate 決策降級`),
      detail: copy(locale, 'The orchestrator continued with reduced decision confidence. Review the gate rationale in the run evidence.', 'Orchestrator 在較低決策信心下繼續執行。請查看執行證據中的 gate 理由。'),
    });
  }
  return findings;
}
...
function translatedRunStatus(status: string, locale: HealthLocale): string {
  if (locale !== 'zh-TW') return status;
  return ({
    created: '已建立', queued: '排隊中', running: '執行中', gating: '審查中', done: '完成',
    failed: '失敗', aborted: '已中止', killed: '已終止', stalled: '停滯', skipped: '已略過',
