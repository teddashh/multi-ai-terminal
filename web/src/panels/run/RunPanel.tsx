...
import { AgentChip, Collapsible, ModalDialog, PROVIDER_COLORS } from '../../components/index.js';
import { displayGateAction, displayNodeError, displayNodeLabel, displayStageName, displaySteerMode, displaySteerStatus } from '../../i18n/displayText.js';
...
export function RunPanel() {
  ...
  const doKill = async (nodeRunId: string) => {
    ...
    catch (error) { setActionError(errorMessage(error, requestFailed)); }
  };
  const openPatch = (node: NodeRun) => {
    ...
    void apiClient.getPatch(patchRunId, node.nodeRunId)
      ...
      .catch((error) => setPatchDialog((current) => current?.runId === patchRunId && current.node.nodeRunId === node.nodeRunId ? { ...current, loading: false, error: errorMessage(error, requestFailed) } : current));
  };
  const applyPatch = async () => {
    ...
    try {
      ...
      setPatchDialog((current) => current?.runId === patchRunId && current.node.nodeRunId === patchNodeRunId ? { ...current, applying: false, error: errorMessage(error, requestFailed) } : current);
    }
  };
  ...
  const openReport = () => {
    ...
    void apiClient.getReport(reportRunId)
      ...
      .catch((error) => { if (matStore.getState().viewedRunId === reportRunId) setReport({ loading: false, error: errorMessage(error, requestFailed) }); });
  };
  ...
  const downloadDebug = async () => {
    ...
    try {
      ...
      if (matStore.getState().viewedRunId === debugRunId) setActionError(errorMessage(error, requestFailed));
    } finally {
  ...
  const card = (node: NodeRun) => <NodeCard
    key={node.nodeRunId}
    node={node}
    displayLabel={displayNodeLabel(node, run.nodes, run.workflow, locale)}
  ...
  return <section className="h-full overflow-auto p-3" aria-label={t('run.panel')}>
    ...
    <div className="space-y-4">
      {run.workflow.stages.map((stage) => {
        ...
        return <section key={stage.id} aria-labelledby={`stage-${stage.id}`}>
          <header className="mb-2 flex items-center justify-between gap-2">
            <div className="min-w-0"><h3 id={`stage-${stage.id}`} className="truncate text-xs font-semibold text-ink">{displayStageName(run.workflow, stage, locale)}</h3><span className="text-[10px] text-muted">{t('run.nodes', { count: stageNodes.length, unit: t(stageNodes.length === 1 ? 'run.node' : 'run.nodePlural') })}</span></div>
      ...
      {steerGroupList.map(({ steer, nodes, decisions }) => <section key={steer.steerId} aria-labelledby={`steer-${steer.steerId}`} data-testid="steer-group">
        <header className="mb-2">
          <div className="flex min-w-0 items-center gap-1.5"><h3 id={`steer-${steer.steerId}`} className="shrink-0 text-xs font-semibold text-accentForeground">{steer.steerStageId ? `${t('run.steer')} ${steer.steerStageId.split('-').at(-1)}` : t('run.steer')}</h3><span className="rounded bg-accentSoft px-1.5 py-0.5 text-[10px] text-accentForeground">{displaySteerMode(steer.mode, locale)}</span><span className="rounded bg-raised px-1.5 py-0.5 text-[10px] text-ink">{displaySteerStatus(steer.status, locale)}</span></div>
    ...
    <ModalDialog open={patchDialog !== undefined} title={patchDialog ? `${t('run.patch')} · ${displayNodeLabel(patchDialog.node, run.nodes, run.workflow, locale)}` : t('run.patch')} onClose={() => setPatchDialog(undefined)} footer={patchDialog && !patchDialog.loading && !patchDialog.error ? <button type="button" disabled={patchDialog.applying} onClick={() => void applyPatch()} className="rounded bg-emerald-700 px-3 py-1.5 text-sm text-white hover:bg-emerald-600 disabled:opacity-50">{patchDialog.applying ? t('run.applying') : t('run.applyPatch')}</button> : undefined}>
    ...
    <ModalDialog open={retryStage !== undefined} title={retryStage ? t('run.retryNamed', { name: displayStageName(run.workflow, retryStage, locale) }) : t('run.retryStage')} onClose={() => setRetryStage(undefined)} footer={<button type="button" onClick={() => void submitRetry()} className="rounded bg-accent px-3 py-1.5 text-sm text-onAccent hover:brightness-110">{t('run.retryStage')}</button>}>
...
export function NodeCard({ node, displayLabel = node.label, latestEvent, seedPrompt, now, confirmingKill, onRequestKill, onCancelKill, onKill, onPatch, onFocus }: NodeCardProps) {
  ...
  return <article data-node-run-id={node.nodeRunId} className="rounded-md border bg-surface/65 p-2.5 shadow-sm" style={{ borderColor: PROVIDER_COLORS[node.agent.provider] }}>
    ...
    {(node.status === 'failed' && (node.errorReason || node.error))
      ? <p className={`mb-2 whitespace-pre-line break-words text-xs ${node.errorReason ? 'text-amber-200' : 'line-clamp-3 text-red-300'}`} title={displayNodeError(node.errorReason ?? node.error ?? '', locale)}>{displayNodeError(node.errorReason ?? node.error ?? '', locale)}</p>
      : (node.status === 'killed' && node.error)
        ? <p className="mb-2 line-clamp-3 break-words text-xs text-red-300" title={displayNodeError(node.error, locale)}>{displayNodeError(node.error, locale)}</p>
...
function VerificationBadge({ node }: { node: NodeRun }) {
  ...
  if (verification.status === 'failed' || verification.status === 'error') {
    return <span title={verification.reason ?? (verification.exitCode === undefined ? t('run.checksFailedTitle') : t('run.exitCode', { code: String(verification.exitCode) }))} className="mb-2 inline-flex rounded-none bg-red-950 px-1.5 py-0.5 text-[10px] text-red-300">{t('run.checksFailed')}</span>;
  }
...
function UsageSummary({ node }: { node: NodeRun }) {
  ...
  return <span className="ml-auto truncate">{tokens > 0 ? t('run.tokens', { count: tokens.toLocaleString(locale) }) : ''}{tokens > 0 && node.usage.costUsd !== undefined ? ' · ' : ''}{node.usage.costUsd !== undefined ? `$${node.usage.costUsd.toFixed(4)}` : ''}</span>;
}

export function DecisionCard({ decision, run }: { decision: GateDecision; run: RunSnapshot }) {
  const { locale, t } = useUiPreferences();
  const display = decisionDisplay(decision);
  const labels = new Map(run.nodes.map((node) => [node.nodeRunId, displayNodeLabel(node, run.nodes, run.workflow, locale)]));
