...
export function NarrativePanel({ loading = false, loadError, visible = true }: { loading?: boolean; loadError?: string; visible?: boolean }) {
  ...
  if (!run) return <section aria-label="Narrative panel" className="grid h-full place-items-center p-6 text-sm text-muted"><div className="max-w-sm text-center"><p className="font-medium text-ink">No run selected</p><p className="mt-2 text-xs">Choose a workspace, select a mode, and start a task. Agent answers will appear here.</p></div></section>;

  return <section aria-label="Narrative panel" className="flex h-full min-h-0 flex-col bg-zinc-950/30">
    <header className="shrink-0 border-b border-border bg-panel/95 px-4 py-3">
      <div className="flex items-start gap-3"><div className="min-w-0 flex-1"><p className="text-[10px] font-semibold uppercase tracking-widest text-violet-300">Conversation</p><p className="mt-1 line-clamp-2 text-sm text-ink" title={run.task}>{run.task}</p></div><span className="shrink-0 rounded-full border border-border px-2 py-1 text-[10px] text-muted">{items.length} blocks</span></div>
      <div className="mt-3 flex items-center gap-2">
        <label className="min-w-0 flex-1"><span className="sr-only">Search conversation</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search answers and evidence" className="w-full rounded border border-border bg-zinc-950 px-2 py-1.5 text-xs text-ink outline-none focus:border-accent" /></label>
        <button type="button" aria-pressed={showTechnical} onClick={() => setShowTechnical((value) => !value)} className={`shrink-0 rounded border px-2 py-1.5 text-[10px] ${showTechnical ? 'border-violet-600 bg-violet-950/30 text-violet-200' : 'border-border text-muted'}`}>Tools & thinking</button>
      </div>
      {focusedNodeRunId && <div className="mt-2 flex items-center gap-2 rounded bg-sky-950/30 px-2 py-1 text-[11px] text-sky-200"><span className="truncate">Focused: {run.nodes.find((node) => node.nodeRunId === focusedNodeRunId)?.label ?? focusedNodeRunId}</span><button type="button" onClick={() => focusNode(undefined)} className="ml-auto rounded px-1.5 py-0.5 hover:bg-sky-900">Show all</button></div>}
      {integrity?.status === 'recovering' && <div role="status" className="mt-2 rounded border border-amber-800 bg-amber-950/30 px-2 py-1.5 text-xs text-amber-200">Recovering missing evidence{integrity.expectedSeq ? ` from event ${integrity.expectedSeq}` : ''}…</div>}
      {integrity?.status === 'incomplete' && <div role="alert" className="mt-2 flex items-center gap-2 rounded border border-red-800 bg-red-950/30 px-2 py-1.5 text-xs text-red-200"><span className="min-w-0 flex-1">Live evidence is incomplete. {integrity.message}</span>{viewedRunId && <button type="button" onClick={() => retryRecovery(viewedRunId)} className="shrink-0 rounded border border-red-700 px-2 py-1">Retry</button>}</div>}
      {loadError && <p role="alert" className="mt-2 text-xs text-red-300">{loadError}</p>}
    </header>
    {loading && events.length === 0 ? <div className="animate-pulse p-4 text-xs text-muted">Loading evidence…</div>
      : events.length === 0 ? <div className="flex items-center gap-2 p-4 text-xs text-muted"><span className="h-2 w-2 animate-pulse rounded-full bg-sky-400" />Waiting for the first agent response…</div>
      : <div ref={scrollRef} onScroll={onScroll} onWheel={() => { userIntentUntil.current = performance.now() + 1500; }} onPointerDown={() => { userIntentUntil.current = performance.now() + 1500; }} className="relative min-h-0 flex-1 overflow-auto overscroll-contain" data-testid="narrative-scroll-region">
        {(events[0]?.seq ?? 1) > 1 && <div role="status" className="border-b border-amber-900/60 bg-amber-950/30 px-4 py-2 text-xs text-amber-200">Older evidence is outside the in-memory window — showing from event {events[0]!.seq}.</div>}
        ...
        {items.length === 0 && <p className="absolute inset-x-0 top-0 p-6 text-center text-xs text-muted">No conversation blocks match these filters.</p>}
...
export function NarrativeRow({ item }: { item: NarrativeItem }) {
  if (item.kind === 'gap') return <article role="alert" className="rounded border border-red-800/70 bg-red-950/25 px-3 py-2 text-xs text-red-200">Evidence gap: events {item.gap?.fromSeq}–{item.gap?.toSeq} are not present in this view.</article>;
  const actor = item.actors[0];
  const providerColor = actor?.provider ? PROVIDER_COLORS[actor.provider] : '#71717a';
  const tone = {
    message: 'border-zinc-700 bg-zinc-900/65', decision: 'border-emerald-700 bg-emerald-950/25', verification: 'border-sky-800 bg-sky-950/20',
    error: 'border-red-800 bg-red-950/30', tool: 'border-amber-800 bg-amber-950/20', thinking: 'border-violet-800 bg-violet-950/20',
    prompt: 'border-sky-800 bg-sky-950/15', lifecycle: 'border-zinc-800 bg-zinc-950/50', gap: 'border-red-800 bg-red-950/30',
  }[item.kind];
  const content = item.kind === 'tool' ? <Collapsible summary={<span>{item.tool?.name ?? 'Tool'} · {item.tool?.phase}</span>}><pre className="mt-2 max-h-80 overflow-auto whitespace-pre-wrap break-words text-[11px] text-amber-100">{[item.tool?.input, item.tool?.output, item.text].filter(Boolean).join('\n\n')}</pre></Collapsible>
    ...
    : <p className={`whitespace-pre-wrap break-words ${item.kind === 'error' ? 'text-red-100' : item.kind === 'decision' ? 'text-emerald-100' : 'text-zinc-100'}`}>{item.text}</p>;
  return <article data-narrative-key={item.key} data-node-run-id={actor?.nodeRunId ?? undefined} className={`rounded-lg border p-3 shadow-sm ${tone}`}>
    <header className="mb-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px] text-muted"><span className="h-2 w-2 rounded-full" style={{ backgroundColor: providerColor }} aria-hidden="true" /><strong className="text-xs text-ink">{actor?.label ?? 'Run'}</strong>{actor?.provider && <span>{actor.provider}{actor.model ? ` · ${actor.model}` : ''}</span>}{actor?.stageName && <span className="rounded bg-zinc-800 px-1.5 py-0.5">{actor.stageName}</span>}<span>attempt {item.attempt}</span><span>#{item.seqStart}{item.seqEnd !== item.seqStart ? `–${item.seqEnd}` : ''}</span><time className="ml-auto" dateTime={new Date(item.tsStart).toISOString()}>{new Date(item.tsStart).toLocaleTimeString(undefined, { hour12: false })}</time></header>
    {content}
  </article>;
}
