...
export function EventRow({ event }: { event: AgentEvent }) {
  ...
  return <article data-event-id={event.id} className={`border-l-2 px-3 py-2 text-sm ${style}`}>
    <header className="mb-1 flex items-center gap-2 text-[11px] not-italic text-muted"><span className="font-semibold">{heading}</span><span>a{event.attempt}</span><span>{event.kind}</span><time dateTime={new Date(event.ts).toISOString()}>{new Date(event.ts).toLocaleTimeString()}</time></header>
