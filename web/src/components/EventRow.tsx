...
export function EventRow({ event, duplicateCount }: { event: AgentEvent; duplicateCount?: number }) {
  ...
  return <article data-event-id={event.id} className={`border-l-2 px-3 py-2 text-sm ${style}`}>
    <header className="mb-1 flex items-center gap-2 text-[11px] not-italic text-muted"><span className="font-semibold">{heading}</span><span>a{event.attempt}</span><span>{event.kind}</span>{duplicateCount && duplicateCount > 1 ? <span className="rounded-full border border-sky-800 px-1.5 py-0.5 text-[10px] text-sky-300">×{duplicateCount} nodes</span> : null}<time dateTime={new Date(event.ts).toISOString()}>{new Date(event.ts).toLocaleTimeString(undefined, { hour12: false })}</time></header>
