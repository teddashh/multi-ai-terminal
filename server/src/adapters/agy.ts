...
const failedSpawn = (message: string, io: Parameters<Adapter['spawn']>[1]): SpawnedNode => {
  ...
  return {
    pid: process.pid,
    kill: () => undefined,
    completion: Promise.resolve({ exitCode: null, error: message }),
  };
};

export function spawnAgy(spec: ResolvedNodeSpec, io: Parameters<Adapter['spawn']>[1]): SpawnedNode {
  ...
  const managed = spawnManaged({ command: 'agy', args, cwd: spec.cwd });
  const { child } = managed;
  let resultText = '';
