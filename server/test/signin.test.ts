...
import {
  ...
  cancelSignIn, providerSignInDescriptor, resetSignInForTests, setSignInRecipeForTests,
...
process.stdin.on('data', (chunk) => {
  buffer += chunk;
  if (!buffer.includes('\\n')) return;
  if (behavior === 'reject-code') { console.log('Error: invalid authorization code'); process.exit(1); }
...
const FAKE_CODEX = `
const args = process.argv.slice(2).join(' ');
if (args === 'login status') { console.log('Logged in using ChatGPT'); process.exit(0); }
...
describe('paste-code ceremony (claude recipe)', () => {
  ...
  it('reports a rejected code when the CLI refuses it', async () => {
    ...
    expect(signInStatus(started.loginId!).phase).toBe('failed');
  }, 30_000);
...
describe('device ceremony (codex recipe)', () => {
  it('surfaces URL and one-time code, then succeeds on the CLI’s own completion', async () => {
    ...
    const status = await settled(started.loginId!);
    expect(status).toMatchObject({ phase: 'succeeded', statusDetail: 'Logged in using ChatGPT' });
  }, 30_000);
...
`);
    ...
    expect(started.outputExcerpt).toContain('evil.attacker.example');
    const status = await settled(started.loginId!);
    expect(status).toMatchObject({ phase: 'failed', error: SIGNIN_CANCELLED });
