...
describe('mock adapter', () => {
  ...
  it('emits the Grok-style auth fixture and exits one for MOCK_AUTHFAIL', async () => {
    const events: AdapterContentEvent[] = [];
    const raw: string[] = [];
    const spawned = mockAdapter.spawn({ binding: { provider: 'mock', model: 'ok', permission: 'safe' }, promptText: 'MOCK_AUTHFAIL', cwd: '/' }, { onEvent: (event) => events.push(event), onRaw: (line) => raw.push(line) });
    await expect(spawned.completion).resolves.toMatchObject({ exitCode: 1 });
    expect(raw.join('\n')).toContain('grok login --device-code');
    expect(events[0]?.text).toContain('Not signed in');
  });
